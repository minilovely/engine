...
namespace oguna
{
	...
	public:
		...
		{
			int size;
			size = ::MultiByteToWideChar(932, MB_PRECOMPOSED, src, length, NULL, 0);
			if (size <= 0) {
				out->clear();
				return 0;
			}
			buffer.resize(size * sizeof(wchar_t));
			MultiByteToWideChar(932, MB_PRECOMPOSED, src, length, (LPWSTR)buffer.data(), size);
		...
		{
			int size;
			size = ::MultiByteToWideChar(CP_UTF8, 0, src, length, NULL, 0);
			buffer.resize(size * sizeof(wchar_t));
			MultiByteToWideChar(CP_UTF8, 0, src, length, (LPWSTR)buffer.data(), size);
			*out = std::wstring((wchar_t*)buffer.data(), size);
			return size;
		}
